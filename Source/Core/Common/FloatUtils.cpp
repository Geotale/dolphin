...
// Used to ensure no weird stuff is going on with the above version
const std::array<BaseAndDec, 32> fres_expected_verify = {{
...
double ApproximateReciprocalVerify(const UReg_FPSCR& fpscr, double val)
{
  ...
  const auto& entry = fres_expected_verify[i / 1024];
