...
inline FPResult NI_madd_msub(PowerPC::PowerPCState& ppc_state, double a, double c, double b)
{
  ...
  {
    // Dummy comment for temporary implementation of this operation, based on previous but fixed
    const double c_round = Force25Bit(c);

    const double b_sign = sub ? -b : b;

    const u32 a_float_bits = ConvertToSingle(std::bit_cast<u64>(a));
    const u32 b_float_bits = ConvertToSingle(std::bit_cast<u64>(b_sign));
    const u32 c_float_bits = ConvertToSingle(std::bit_cast<u64>(c_round));

    const u64 a_conv_bits = ConvertToDouble(a_float_bits);
    const u64 b_conv_bits = ConvertToDouble(b_float_bits);
    const u64 c_conv_bits = ConvertToDouble(c_float_bits);

    if (a_conv_bits == std::bit_cast<u64>(a) &&
        b_conv_bits == std::bit_cast<u64>(b_sign) &&
        c_conv_bits == std::bit_cast<u64>(c_round))
    {
      const float a_float = std::bit_cast<float>(a_float_bits);
      const float b_float = std::bit_cast<float>(b_float_bits);
      const float c_float = std::bit_cast<float>(c_float_bits);

      result.value = static_cast<double>(std::fma(a_float, c_float, b_float));
    }
    else
    {
